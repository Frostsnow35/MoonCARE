...
class NLPService:
    ...
    async def analyze_text(self, text: str) -> Dict:
        ...
        # Calculate multidimensional emotion state for routing and memory context.
        emotion_dimensions = self._calculate_emotion_dimensions(emotion_scores, text_lower)
        emotion_intensity = self._calculate_emotion_intensity(emotion_scores, emotion_dimensions)
    ...
    def _calculate_emotion_dimensions(self, emotion_scores: Dict[str, float], text_lower: str) -> Dict[str, float]:
        """
        计算多维情绪负荷。

        这些字段用于陪伴语境和路由辅助，不作为医学或心理诊断。
        """
        body_terms = ["头痛", "腹痛", "肚子痛", "乳房胀痛", "腹胀", "水肿", "失眠", "cramps", "headache"]
        body_load = min(sum(1 for term in body_terms if term in text_lower) / 3, 1.0)
        anxiety_load = min(emotion_scores.get("anxious", 0.0) + emotion_scores.get("fear", 0.0) * 0.6, 1.0)
        sadness_load = min(emotion_scores.get("sad", 0.0), 1.0)
        irritability_load = min(emotion_scores.get("angry", 0.0), 1.0)
        stress_load = min(emotion_scores.get("stressed", 0.0), 1.0)
        fatigue_load = min(emotion_scores.get("tired", 0.0), 1.0)
        calm_load = min(emotion_scores.get("calm", 0.0), 1.0)
        positive_load = min(emotion_scores.get("happy", 0.0) + calm_load * 0.7, 1.0)
        negative_load = min(
            anxiety_load + sadness_load + irritability_load + stress_load + fatigue_load + body_load * 0.5,
            1.0,
        )
        arousal = min(anxiety_load + irritability_load + stress_load + emotion_scores.get("fear", 0.0), 1.0)
        valence = max(-1.0, min(1.0, positive_load - negative_load))

        return {
            "valence": round(valence, 2),
            "arousal": round(arousal, 2),
            "anxiety_load": round(anxiety_load, 2),
            "sadness_load": round(sadness_load, 2),
            "irritability_load": round(irritability_load, 2),
            "stress_load": round(stress_load, 2),
            "fatigue_load": round(fatigue_load, 2),
            "body_load": round(body_load, 2),
        }
    ...
    def _calculate_emotion_intensity(
        ...
        """Return a conservative 0-1 estimate of current emotional intensity."""
        if not emotion_scores:
            return 0.0
        dominant_score = max(emotion_scores.values())
        load_score = max(
            emotion_dimensions.get("anxiety_load", 0.0),
            emotion_dimensions.get("sadness_load", 0.0),
            emotion_dimensions.get("irritability_load", 0.0),
            emotion_dimensions.get("stress_load", 0.0),
            emotion_dimensions.get("fatigue_load", 0.0),
            emotion_dimensions.get("body_load", 0.0),
        )
        combined_load = min(
            emotion_dimensions.get("anxiety_load", 0.0)
            + emotion_dimensions.get("stress_load", 0.0)
        ...
        return max(0.0, min(1.0, dominant_score * 0.45 + load_score * 0.35 + combined_load * 0.20))
