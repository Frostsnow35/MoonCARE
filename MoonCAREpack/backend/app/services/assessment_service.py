...
class AssessmentOrchestrator:
    ...
    def _is_triggered(self, message: str, context: Dict[str, Any]) -> bool:
        ...
        symptom_terms = ["烦躁", "胀痛", "失眠", "想哭", "低落", "乳房胀", "腹痛", "腰酸", "水肿"]
        if any(term in text for term in menstrual_terms):
            return True
        support_context = (context or {}).get("support_context") or {}
        if support_context.get("menstrual_related") and any(term in text for term in symptom_terms):
            return True
        if (context or {}).get("cycle_phase") in {"luteal", "经前期", "黄体期"} and any(
            term in text for term in symptom_terms
        ):
            return True
        return False
