...
class AgentService:
    ...
    def _looks_like_contextual_knowledge_question(self, user_message: str, state: Dict[str, Any]) -> bool:
        """Return whether recent context turns a short follow-up into a knowledge question."""
        router = self._get_router()
        if hasattr(router, "_looks_like_contextual_knowledge_question"):
            return router._looks_like_contextual_knowledge_question(user_message, state)
        return False
    ...
    async def get_streaming_response(
        ...
        try:
            ...
            if (
                ...
                or self._looks_like_knowledge_question(user_message)
                or self._looks_like_contextual_knowledge_question(user_message, state)
            ):
                router = self._get_router()
