...
class AgentService:
    ...
    def _attach_conversation_context(
        ...
        state["user_profile"] = dict((context or {}).get("user_profile") or {})
        state["user_nickname"] = str((context or {}).get("user_nickname") or "").strip()
        state["is_first_assistant_turn"] = bool((context or {}).get("is_first_assistant_turn"))
    ...
    def _nickname_prompt_hint(self, state: Dict[str, Any]) -> str:
        """Tell the support prompt when a one-time nickname address is appropriate."""
        state = state or {}
    ...
    def _build_support_stream_context(
        ...
        context = {
            ...
            "is_first_assistant_turn": bool(state.get("is_first_assistant_turn")),
        }
        if render_prompt:
            ...
            context["raw_system_prompt"] = f"{raw_system_prompt}{self._nickname_prompt_hint(state)}"
        return context
