...
class P0ChatWebSocketAuthTests(unittest.TestCase):
    ...
    def _create_user(self, email: str, nickname: str | None = None) -> tuple[int, str, dict[str, str]]:
    ...
    def test_websocket_uses_token_identity_not_path_user_id(self):
        ...
        with self.client.websocket_connect(f"/api/v1/chat/ws/{path_user_id}?token={token}") as websocket:
            ...
            websocket.send_json({"message": "today is hard", "agent_mode": "support"})
    ...
    def test_websocket_accepts_two_messages_in_same_session(self):
        ...
        with self.client.websocket_connect(f"/api/v1/chat/ws?token={token}") as websocket:
            ...
            websocket.send_json({"message": "I feel bad", "agent_mode": "support"})
            first_reply = websocket.receive_json()
            self.assertEqual(first_reply["type"], "assistant")

            websocket.send_json({"message": "still bad", "agent_mode": "support"})
    ...
    def test_rest_accepts_two_messages_in_same_session(self):
        ...
        first_response = self.client.post(
            "/api/v1/chat/message",
            data={"message": "I feel bad", "agent_mode": "support"},
            headers=headers,
        )
        ...
        second_response = self.client.post(
            "/api/v1/chat/message",
            data={"message": "still bad", "session_id": session_id, "agent_mode": "support"},
            headers=headers,
        )
    ...
    def test_first_rest_reply_addresses_user_nickname(self):
        _user_id, _token, headers = self._create_user(
            "nickname-first@example.com",
            nickname="adminMC",
        )
        ...
        self.assertIn("adminMC", response.json()["reply"])

    def test_first_stream_reply_addresses_user_nickname(self):
        _user_id, _token, headers = self._create_user(
            "nickname-stream@example.com",
            nickname="adminMC",
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn("adminMC", response.text)
