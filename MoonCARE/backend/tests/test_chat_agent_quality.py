...
class ChatApiContractTests(unittest.TestCase):
    ...
    def test_frontend_chat_no_longer_uses_legacy_interview_mode(self):
        chat_view = (BACKEND_ROOT.parent / "frontend" / "src" / "views" / "Chat.vue").read_text(encoding="utf-8")

        self.assertNotIn("interviewAPI", chat_view)
        self.assertNotIn("isInterviewMode", chat_view)
        self.assertNotIn("result.report", chat_view)

    def test_home_notifications_route_points_to_existing_profile_screen(self):
        home_view = (BACKEND_ROOT.parent / "frontend" / "src" / "views" / "Home.vue").read_text(encoding="utf-8")
        router_file = (BACKEND_ROOT.parent / "frontend" / "src" / "router" / "index.js").read_text(encoding="utf-8")

        self.assertIn("router.push('/profile')", home_view)
        self.assertNotIn("router.push('/profile/notifications')", home_view)
        self.assertNotIn("path: '/profile/notifications'", router_file)

    def test_wave_monitor_uses_demo_flag_and_no_default_seed_or_mock(self):
        wave_view = (BACKEND_ROOT.parent / "frontend" / "src" / "views" / "WaveMonitor.vue").read_text(encoding="utf-8")

        self.assertIn("VITE_ENABLE_BIOMETRIC_DEMO", wave_view)
        self.assertNotIn("biometricAPI.seed(50)", wave_view)
        self.assertNotIn("Generate mock data for demo", wave_view)
        self.assertNotIn("Fallback to mock data on error", wave_view)

    def test_android_shell_uses_manifest_placeholders_for_backup_and_cleartext(self):
        manifest = (
            BACKEND_ROOT.parent
            / "frontend"
            / "android"
        ...
        self.assertIn('android:allowBackup="${allowBackup}"', manifest)
        self.assertIn('android:usesCleartextTraffic="${usesCleartextTraffic}"', manifest)
        self.assertIn('manifestPlaceholders = [', app_gradle)
        self.assertIn('usesCleartextTraffic: "true"', app_gradle)
        self.assertIn('allowBackup: "false"', app_gradle)

    def test_app_update_store_supports_force_update_and_manual_checks(self):
        update_store = (
            BACKEND_ROOT.parent
            / "frontend"
            / "src"
        ...
        self.assertTrue(update_store.exists())

        store_text = update_store.read_text(encoding="utf-8")
        self.assertIn("defineStore('appUpdate'", store_text)
        self.assertIn("async function checkForUpdates", store_text)
        self.assertIn("force_update", store_text)
        self.assertIn("min_supported_version_code", store_text)
        self.assertIn("downloadAndInstall", store_text)

    def test_profile_exposes_version_and_update_actions(self):
        profile_view = (
            BACKEND_ROOT.parent
            / "frontend"
            / "src"
        ...
        self.assertIn("版本与更新", profile_view)
        self.assertIn("useAppUpdateStore", profile_view)
        self.assertIn("checkForUpdates", profile_view)
        self.assertIn("startUpdate", profile_view)
        self.assertIn("最近检查", profile_view)

    def test_app_shell_branding_uses_mooncare_not_healthai(self):
        app_view = (
            BACKEND_ROOT.parent
            / "frontend"
            / "src"
        ...
        self.assertIn("MoonCARE", app_view)
        self.assertNotIn("HealthAI", app_view)

    def test_android_release_build_has_flavors_and_signing_entrypoints(self):
        app_gradle = (
            BACKEND_ROOT.parent
            / "frontend"
            / "android"
        ...
        self.assertIn("signingConfigs", app_gradle)
        self.assertIn("flavorDimensions", app_gradle)
        self.assertIn("productFlavors", app_gradle)
        self.assertIn("internal {", app_gradle)
        self.assertIn("public {", app_gradle)
        self.assertIn("keystore.properties", app_gradle)
        self.assertIn("MOONCARE_UPLOAD_STORE_FILE", app_gradle)
        self.assertIn("keystore.properties", android_gitignore)

    def test_android_debug_build_is_not_blocked_by_release_signing_guard(self):
        app_gradle = (
            BACKEND_ROOT.parent
            / "frontend"
            / "android"
        ...
        self.assertNotIn("gradle.taskGraph.whenReady", app_gradle)
        self.assertNotIn("Release signing config missing.", app_gradle)
        self.assertIn("if (hasReleaseSigningConfig)", app_gradle)

    def test_android_gradle_chain_is_upgraded_for_modern_android_studio(self):
        android_root = BACKEND_ROOT.parent / "frontend" / "android"
        project_gradle = (android_root / "build.gradle").read_text(encoding="utf-8")
        wrapper_properties = (
            android_root / "gradle" / "wrapper" / "gradle-wrapper.properties"
        ).read_text(encoding="utf-8")
        variables_gradle = (android_root / "variables.gradle").read_text(encoding="utf-8")
        capacitor_app_gradle = (
            android_root / "app" / "capacitor.build.gradle"
        ).read_text(encoding="utf-8")

        self.assertIn("com.android.tools.build:gradle:8.10.0", project_gradle)
        self.assertIn("gradle-8.11.1-all.zip", wrapper_properties)
        self.assertIn("compileSdkVersion = 36", variables_gradle)
        self.assertIn("targetSdkVersion = 36", variables_gradle)
        self.assertIn("buildToolsVersion = '36.1.0'", variables_gradle)
        self.assertIn("JavaVersion.VERSION_17", capacitor_app_gradle)
        self.assertNotIn("JavaVersion.VERSION_21", capacitor_app_gradle)

    def test_android_internal_update_plugin_and_permission_are_wired(self):
        main_activity = (
            BACKEND_ROOT.parent
            / "frontend"
            / "android"
        ...
        self.assertIn("registerPlugin(MooncareUpdaterPlugin.class)", main_activity)
        self.assertTrue(plugin_file.exists())
        self.assertTrue(internal_manifest.exists())
        self.assertIn(
            "REQUEST_INSTALL_PACKAGES",
            internal_manifest.read_text(encoding="utf-8"),
        )

    def test_android_release_tooling_docs_and_script_exist(self):
        release_script = (
            BACKEND_ROOT.parent
            / "frontend"
            / "scripts"
            / "prepare_android_release.py"
        )
        release_doc = (
            BACKEND_ROOT.parent
            / "docs"
            / "android-self-hosted-updates.md"
        )

        self.assertTrue(release_script.exists())
        self.assertTrue(release_doc.exists())
        self.assertIn("sha256", release_script.read_text(encoding="utf-8"))
        self.assertIn("android-{channel}.json", release_script.read_text(encoding="utf-8"))
        self.assertIn("MOBILE_RELEASES_PUBLIC_BASE_URL", release_doc.read_text(encoding="utf-8"))
