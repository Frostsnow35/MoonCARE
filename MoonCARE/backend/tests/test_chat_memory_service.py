...
class ChatMemoryServiceTests(unittest.TestCase):
    ...
    def test_product_memory_state_uses_local_database_only(self):
        from app.services.product_memory_service import ProductMemoryService

        service = ProductMemoryService(self.db)
        prompt_context = service.build_prompt_context(user_id=1, session_id="session-local-memory")

        memory_state = prompt_context["memory_state"]
        self.assertEqual(memory_state["provider"], "local_db")
        self.assertNotIn("awareness_available", memory_state)
        self.assertNotIn("awareness_recalled", memory_state)
        self.assertNotIn("awareness_items", memory_state)
        self.assertNotIn("fallback_reason", memory_state)
