...
const mobile_release_base_url = (
  import.meta.env.VITE_APP_UPDATE_CHECK_URL ||
  `${api_base_url}/mobile/releases/android`
).replace(/\/+$/, '')
...
const mobileReleaseClient = axios.create({
  baseURL: mobile_release_base_url,
  timeout: 15000,
  headers: {
    'Content-Type': 'application/json'
  }
...
mobileReleaseClient.interceptors.response.use(
  response => response.data,
  error => {
    console.error('Mobile Release API Error:', error)
    return Promise.reject(error)
  }
)
...
export const mobileAPI = {
  getAndroidRelease: (channel) => mobileReleaseClient.get(channel)
}
